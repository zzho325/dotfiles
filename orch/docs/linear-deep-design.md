...
| Split Linear tab into top tree + bottom detail block | Forces a third focus level (which split has cursor); `linear-tui-design.md` §6 admitted this was the contentious part of the prior design |
...
- The transition modal in `linear-tui-design.md` §7 was three modals deep
  (transition → state list → optimistic ack with revert); each one is a
  failure surface
