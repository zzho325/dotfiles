...
Field order, fixed workspaces where it matters:
...
Same row anatomy, but workspace 0–4 is the tree glyph instead of cursor:
...
…actually simpler: align the sub-issue's `key` to the **same workspace as
parent keys** (col 3), so the eye scans one workspace of `ENG-…` keys
