...
Single workspace (Column). One linked issue per task is the common case;
...
away when richer context is needed. See `linear-tui-design.md` for the fuller
exploration that this minimal version was distilled from.
...
worktrees when they exit, so `~/column/worktrees/` accumulates orphans.
