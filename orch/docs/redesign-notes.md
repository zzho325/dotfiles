...
- [ ] `orch gc` subcommand: scans the configured worktree root, lists worktrees
...
- [ ] Detail tabs (selected task = #2 with full fixture content):
  - [ ] `snapshot_detail_tab_overview` — fixture (full Overview field set):
        ...
        - `worktree.path`: `~/worktrees/infra-triage`
...
- [ ] Misc:
  ...
  - [ ] `snapshot_message_input` — fixture: `m` pressed on selected
        ...
        TUI as a single-line modal with cursor at workspace 0, addressed to
...
- **Q1** Authoritative Linear doc: `redesign.md` §4. Broader Linear UI
  explorations were cut from the repo.
