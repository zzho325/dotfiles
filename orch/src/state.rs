...
/// Ensure the task has a worktree. Missing worktrees are created from a
/// clean, fast-forwarded `$ORCH_REPO/main`. Existing dirty worktrees are
/// allowed only for already-started tasks, so resume preserves WIP but new
/// tasks don't silently reuse stale local changes.
pub fn prepare_task_worktree(
    name: &str,
    recorded_worktree: &str,
    allow_existing_dirty: bool,
) -> Result<String, String> {
    let repo = std::env::var("ORCH_REPO")
        .map_err(|_| "ORCH_REPO not set".to_string())?;
    let repo = PathBuf::from(repo);
    let worktree = if recorded_worktree.is_empty() {
        repo.join(format!("task-{name}"))
    } else {
        PathBuf::from(expand_home(recorded_worktree))
    };

    if worktree.exists() {
        if git_has_changes(&worktree)? && !allow_existing_dirty {
            return Err(format!(
                "worktree already exists and is dirty: {}\n\
                 inspect it or remove it before spawning a new worker",
                worktree.display()
            ));
        }
        return Ok(worktree.to_string_lossy().to_string());
    }

    refresh_main_worktree(&repo)?;
    create_task_worktree(&repo, name)?;
    if !worktree.exists() {
        return Err(format!(
...
fn refresh_main_worktree(repo: &Path) -> Result<(), String> {
    ...
    if !main.exists() {
        return Err(format!("main worktree missing: {}", main.display()));
    }
    if git_has_changes(&main)? {
        return Err(format!(
            "main worktree is dirty: {}\n\
             commit/stash/reset it before spawning a new worker",
            main.display()
        ));
    }
    run_git(&main, &["fetch", "origin", "main"])?;
    run_git(&main, &["merge", "--ff-only", "origin/main"])
}

fn create_task_worktree(repo: &Path, name: &str) -> Result<(), String> {
    let main = repo.join("main");
    let worktree = repo.join(format!("task-{name}"));
    ...
    Err(command_error("git worktree add", &output))
}

fn git_has_changes(path: &Path) -> Result<bool, String> {
    let output = Command::new("git")
