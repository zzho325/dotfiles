...
HOMEBREW_NO_AUTO_UPDATE=1 brew bundle install --file=./setup/Brewfile
...
if command -v stow >/dev/null; then
  cd "$HOME/dotfiles"
  stow --adopt --no-folding zsh nvim git jj starship ghostty zellij tmux claude agents codex --target="$HOME"
